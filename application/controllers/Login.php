...
{
    public $result = array();  // Add this line to declare the property
    ...
    public function __construct()
    {
        ...
        $this->result = array();  // Initialize the result array
    }
    ...
    function isLoggedIn()
    {
        ...
        if (!isset($isLoggedIn) || $isLoggedIn != TRUE) {
            ...
            if (!empty($this->input->post('rememberMe'))) {
                ...
                setcookie("system_user_login", $this->input->post('username'), time() + (10 * 365 * 24 * 60 * 60));
                //COOKIES for password
                setcookie("system_userpassword", $this->input->post('password'), time() + (10 * 365 * 24 * 60 * 60));
            } else {
                if (isset($_COOKIE["system_user_login"])) {
                    setcookie("system_user_login", "");
                    if (isset($_COOKIE["system_userpassword"])) {
                        setcookie("system_userpassword", "");
                    }
                }
            }
